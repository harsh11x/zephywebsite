    ...
    <div className="min-h-screen bg-black text-white relative overflow-hidden">
      ...
      <main className="container py-12 sm:py-16 md:py-20">
        <MotionDiv initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          {/* Encryption Tools */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 md:gap-6 lg:gap-8 max-w-4xl sm:max-w-5xl mx-auto px-3 sm:px-4 md:px-0">
            {encryptionTools.map((tool, index) => (
              <div key={tool.title} className="bg-white/10 backdrop-blur-lg border-2 border-white/20 shadow-lg rounded-xl p-4 sm:p-5 md:p-6 group cursor-pointer h-full transition-transform duration-300 hover:bg-white/20 hover:shadow-xl">
                <Link href={tool.href} className="block h-full">
                  <div className="flex flex-col h-full justify-between">
                    <div>
                      ...
                      <div className="text-base sm:text-lg md:text-xl lg:text-2xl font-light text-white mt-2 sm:mt-3 md:mt-4">{tool.title}</div>
                      <div className="text-xs sm:text-sm md:text-base text-white/60 font-light mt-1 sm:mt-2">{tool.description}</div>
                    </div>
