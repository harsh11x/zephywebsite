...
export default function AuthPage() {
  ...
  useEffect(() => {
    console.log("Auth useEffect - user:", user?.email, "selectedPlan:", selectedPlan)
    if (user && !selectedPlan) {
      console.log("Redirecting to dashboard - user signed in, no plan selected")
      router.push("/dashboard")
    }
  }, [user, selectedPlan, router])

  useEffect(() => {
    console.log("Plan useEffect - selectedPlan:", selectedPlan)
    if (selectedPlan) {
      setActiveTab("signup")
    }
  }, [selectedPlan])
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      console.log("Starting Google sign-in with plan:", selectedPlan)
      ...
      if (selectedPlan && selectedPlan !== "free") {
        ...
        // User with paid plan - redirect to payment
        router.push(`/payment?plan=${selectedPlan}&email=${userEmail}`)
      } else {
        console.log("Redirecting to dashboard (free plan or no plan)")
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          {selectedPlan && (
            <div className="order-2 lg:order-1">
              <PlanSummary planId={selectedPlan} />
            </div>
          )}

          {/* Auth Form */}
          <div className={`order-1 ${selectedPlan ? 'lg:order-2' : ''}`}>
